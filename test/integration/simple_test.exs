defmodule Membrane.Whisper.Integration.SimpleTest do
  ...
  test "audio buffers are forwarded without change and transcripts are sent as events", ctx do
    ...
    |> Enum.each(&assert_sink_event(pipeline_pid, :testing_sink, &1, 5_000))

    assert_end_of_stream(pipeline_pid, :testing_sink, :input, 5_000)

    assert_end_of_stream(pipeline_pid, :sink, :input, 5_000)
